    ...
    <Sheet>
      <SheetTrigger asChild>
        <Button 
          variant="outline" 
          className={`${isMobile 
            ? 'fixed bottom-0 left-1/2 -translate-x-1/2 w-[54%] h-16 rounded-b-none rounded-t-lg border-b-0 z-50 bg-white shadow-lg px-4 py-2 mb-0' 
            : 'fixed right-0 top-1/2 -translate-y-1/2 h-32 w-12 px-2 rounded-l-lg rounded-r-none border-r-0 z-50 bg-white shadow-lg'}`}
        >
          {isMobile ? (
            ...
            <div className="flex flex-col items-center justify-center h-full gap-4">
              ...
              <span className="-rotate-90 whitespace-nowrap">Strategy</span>
            </div>
          )}
        </Button>
      </SheetTrigger>
